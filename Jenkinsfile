    ...
    stages {
        ...
        stage('Criação ou atualização da infra') {
            ...
            steps {
                ...
                script {
                    dir('src') {
                        sh 'terraform init --backend-config "resource_group_name=${resource_group_name}" --backend-config "storage_account_name=${liveterraformfv}" --backend-config "container_name=${container_name}" --backend-config "key=${az_key}"'
                    }
                }
            }
        }
